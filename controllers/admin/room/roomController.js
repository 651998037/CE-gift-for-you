    ...
    if (typeof req.session.userid == 'undefined') { res.redirect('/');}else{
        req.getConnection((err, conn) => {
            conn.query('SELECT * FROM room', (err, rooms) => {
                ...
                res.render('./admin/roomView/roomList', {
                    data: rooms,
                    session: req.session
                });
            });
...
controller.add = (req, res) => {
    if (typeof req.session.userid == 'undefined') { res.redirect('/');}else{
        req.getConnection((err, conn) => {
            conn.query('SELECT * FROM room', (err, rooms) => {
                res.render('./admin/roomView/roomAdd', {
                    data1: rooms,
                    session: req.session
                });
            });
    ...
    if (typeof req.session.userid == 'undefined') { res.redirect('/');}else{
        ...
        req.getConnection((err, conn) => {
            conn.query('SELECT * FROM room WHERE id = ?',[url.id], (err, room) => {
                ...
                res.render('./admin/roomView/roomedit', {
                    room,
                    session: req.session
                });
            });
...
controller.save = (req, res) => {
    if (typeof req.session.userid == 'undefined') { res.redirect('/');}else{
        ...
        if (!errors.isEmpty()) {
            req.session.errors = errors;
            req.session.success = false;
            return res.redirect('/room/edit/' + url.id);
        } else {
            req.session.success = true;
...
controller.del = (req, res) => {
    if (typeof req.session.userid == 'undefined') { res.redirect('/');}else{
        ...
        res.render('./admin/roomView/Confirm', {
            data: data,
            session: req.session
        });
    };
