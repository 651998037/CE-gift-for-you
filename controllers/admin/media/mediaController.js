...
controller.show=(req,res) => {
    ...
    req.getConnection((err,conn) =>{
        conn.query('SELECT * FROM media',(err,media)=>{
            ...
            res.render('./admin/mediaView/mediaList',{
...
controller.add = (req, res) => {
    if (typeof req.session.userid == 'undefined') { res.redirect('/');}else{
    res.render('./admin/mediaView/mediaAdd',{
        session:req.session
    });
}};
...
controller.del=(req, res) => {
    ...
    res.render('./admin/mediaView/confirmDel',{
...
controller.edit = (req, res) => {
    ...
    req.getConnection((err, conn) => {
        conn.query('SELECT * FROM media WHERE id = ?', [idToEdit], (err, data) => {
            ...
            res.render('./admin/mediaView/mediaEdit', { data: data[0],session:req.session });
        });
    });
}};
...
controller.save = (req, res) => {
    ...
    if(!errors.isEmpty()){
        req.session.errors=errors;
        req.session.success =false;
        return  res.redirect('/mediaEdit/'+ req.params.id)
    }else{
        req.session.success=true;
