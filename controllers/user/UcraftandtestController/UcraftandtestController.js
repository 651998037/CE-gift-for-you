    ...
    if (typeof req.session.userid == 'undefined') { res.redirect('/');}else{
        req.getConnection((err, conn) => {
            conn.query('SELECT * FROM craftandtest', (err, allcraftandtests) => {
                ...
                res.render('./user/craftandtestView/UcraftandtestList', {
...
controller.add = (req, res) => {
    if (typeof req.session.userid == 'undefined') { res.redirect('/');}else{
        res.render('./user/craftandtestView/UcraftandtestAdd', {
            session: req.session,
...
controller.delete = (req, res) => {
    if (typeof req.session.userid == 'undefined') { res.redirect('/');}else{
        ...
        res.render('./user/craftandtestView/confirmUcraftandtestEdit', {
    ...
    if (typeof req.session.userid == 'undefined') { res.redirect('/');}else{
        ...
        req.getConnection((err, conn) => {
            conn.query('SELECT * FROM craftandtest WHERE id = ?', [idToEdit], (err, data) => {
                ...
                res.render('./user/craftandtestView/UcraftandtestEdit', { data: data[0],session:req.session });
            });
        });
    };
};
